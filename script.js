...
if (productoId) {
 ...
 const aplicarFiltros = () => {
  ...
  cards.forEach(producto => {
    ...
    const vendedor = producto.dataset.vendedor?.toLowerCase() || '';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('beforeinstallprompt', (e) => {
    ...
    const installLi = document.getElementById('install-li');
    const installBtn = document.getElementById('install-btn');

    if (installLi && installBtn) {
      installLi.style.display = 'block'; // 👈 Usa block en vez de list-item

      installBtn.addEventListener('click', async (event) => {
        event.preventDefault();
        deferredPrompt.prompt();
        const result = await deferredPrompt.userChoice;
        console.log(`El usuario eligió: ${result.outcome}`);
        deferredPrompt = null;
